...
export default function Admin() {
    ...
    useEffect(() => {
        async function loadMsgs(clientId) {
            try {
                const { data: response } = await api.get(`/messages/${user.id}/${clientId}/`);
        ...
        return (
            <>
                ...
                <div className={styles.selectContainer}>
                    <select className={styles.select} name="client_id" id="client_id" onClick={() => { setClick(!click) }} onChange={(e) => { setClientId(e.target.value); setReload(true); }}>
                        <option selected="selected">Select a user</option>
                        {clients.map((a, i) => {
                            return (
                                <option key={i} value={a.id}>{a.username}</option>
                            )
                        })}
                    </select>
                </div>
                ...
                <form className={styles.sendMsg} onSubmit={handleSubmit(sendMsg)}>
                    ...
                    <button type="submit" onClick={() => document.getElementById("msg").value = ""}>Send</button>
                </form>
                <Footer />
            </>
        )
